...
const parseTargetDate = (settings: SiteSettings): number => {
  let targetTime = NaN;
...
const Countdown = ({ settings }: { settings: SiteSettings }) => {
  ...
  useEffect(() => {
    const weddingDate = parseTargetDate(settings);
...
const RSVPAndGuestbook = ({ messages, onAddMessage, guestName, onOpenModal }: { messages: Message[], onAddMessage: (data: any) => void, guestName: string, onOpenModal: () => void }) => {
  return (
        ...
        <Route path="/" element={
          ...
          <motion.div
            ...
            animate={{ 
              ...
              filter: isOpened ? "none" : "blur(10px)" 
            }}
            transition={{ 
              duration: 1.5, 
              delay: 0.2,
          ...
          >
            ...
            <RSVPAndGuestbook 
              ...
              guestName={guestName} 
              onOpenModal={() => setIsRSVPModalOpen(true)}
            />
            <Footer settings={siteSettings} />
          </motion.div>

          {/* Fixed-position Modal outside the animated wrapper */}
          <AnimatePresence>
            {isRSVPModalOpen && (
              <WishesModal 
